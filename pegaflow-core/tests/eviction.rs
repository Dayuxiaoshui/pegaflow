...
async fn eviction_reclaims_old_blocks_for_new() {
    ...
    env.data().zero_gpu();
    env.assert_all_hit_and_reserve(&new).await;
    env.load_to_gpu(&new).await;
    env.data().assert_gpu_matches_expected();
}

/// Leased blocks survive memory pressure: reserve_load leases prevent LRU eviction.
#[tokio::test]
async fn leased_blocks_survive_eviction_pressure() {
    ...
    env.assert_all_hit_and_reserve(&hashes).await; // reserves first batch
    ...
    // First batch is still reserved — load must succeed.
    env.data().zero_gpu();
